...
namespace Jackett.Updater
{
    public class Program
    {
        ...
        {
            var runtimeSettings = new RuntimeSettings { CustomLogFileName = "updater.txt" };
            ...
            foreach (var pid in pids)
                try
                {
                    ...
                    if (Environment.OSVersion.Platform == PlatformID.Unix)
                    {
                        try
                        {
                            var startInfo = new ProcessStartInfo { Arguments = $"-15 {pid}", FileName = "kill" };
                            Process.Start(startInfo);
                            Thread.Sleep(
                                1000); // just sleep, WaitForExit() doesn't seem to work on mono/linux (returns immediantly), https://bugzilla.xamarin.com/show_bug.cgi?id=51742
        ...
        private void ProcessUpdate(UpdaterConsoleOptions options)
        {
            ...
            {
                if (isWindows && (trayRunning || options.StartTray) && !string.Equals(
                        options.Type, "WindowsService", StringComparison.OrdinalIgnoreCase))
                {
        ...
        private bool CopyUpdateFile(string jackettDestinationDirectory, string fullSourceFilePath,
                                    string updateSourceDirectory, bool previousAttemptFailed)
        {
            ...
            try
            {
                ...
                fullDestinationFilePath = Path.Combine(
                    jackettDestinationDirectory, fullSourceFilePath.Substring(updateSourceDirectory.Length));
        ...
        {
            var variants = new Variants();
            return variants.IsNonWindowsDotNetCoreVariant(_variant)
                ? Path.Combine(directoryPath, "jackett")
                : Path.Combine(directoryPath, "JackettConsole.exe");
        }
