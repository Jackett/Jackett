...
namespace Jackett.Updater
{
	public class Program
	{
		...
		private IServiceConfigService _windowsService;
		...
		private void Run(string[] args)
		{
			...
			logger.Info($"Jackett variant: {_variant}");
			...
			_windowsService = new WindowsServiceConfigService(_processService, logger);
			...
			foreach (var pid in pids)
				try
				{
					...
					if (Environment.OSVersion.Platform == PlatformID.Unix)
					{
						...
						{
							logger.Error(e, $"Error while sending SIGTERM to {pid}");
						}
				...
				{
					logger.Info($"Error killing process {pid}");
					logger.Info(e);
				}
		}
		...
		private void ProcessUpdate(UpdaterConsoleOptions options)
		{
			...
			var variants = new Variants();
			if (variants.IsNonWindowsDotNetCoreVariant(_variant))
			{
			...
			try
			{
				foreach (var file in from file in files
						...
						where !fileName.EndsWith(".zip") && !fileName.EndsWith(".tar") && !fileName.EndsWith(".gz")
						let fileCopySuccess = CopyUpdateFile(options.Path, file, updateLocation, false)
						where !fileCopySuccess
						select file)
			...
			if (options.NoRestart) return;
			if (isWindows && (trayRunning || options.StartTray) && !string.Equals(options.Type, "WindowsService",
			...
			else
			{
				...
				switch (_variant)
				{
					...
					case Variants.JackettVariant.CoreLinuxAmdx64:
					case Variants.JackettVariant.CoreLinuxArm32:
					case Variants.JackettVariant.CoreLinuxArm64:
		...
		private static bool CopyUpdateFile(string jackettDestinationDirectory, string fullSourceFilePath,
			string updateSourceDirectory, bool previousAttemptFailed)
		{
			var success = false;
			...
			try
			{
				...
				fullDestinationFilePath = Path.Combine(jackettDestinationDirectory,
					fullSourceFilePath.Substring(updateSourceDirectory.Length));
			...
			logger.Info(
				$"Attempting to copy {fileName} from source: {fullSourceFilePath} to destination: {fullDestinationFilePath}");

			if (previousAttemptFailed)
			{
				logger.Info(
					$"The first attempt copying file: {fileName}failed. Retrying and will delete old file first");
		...
		private string GetJackettConsolePath(string directoryPath)
		{
			...
			return Path.Combine(directoryPath,
				variants.IsNonWindowsDotNetCoreVariant(_variant) ? "jackett" : "JackettConsole.exe");
		}
