            ...
            foreach (var title in query.ShowTitles ?? new string[] { string.Empty })
            {
                ...
                try
                {
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        var dateStr = qRow.Find(".time").Text().Split(' ');
                        TimeSpan ts = TimeSpan.Zero;
                        if (dateStr.Contains("min"))
                            ts = TimeSpan.FromMinutes(int.Parse(dateStr[0]));
                        else if (dateStr.Contains("hour"))
                            ts = TimeSpan.FromHours(int.Parse(dateStr[0]));
                        else if (dateStr.Contains("day"))
                            ts = TimeSpan.FromDays(int.Parse(dateStr[0]));
                        else if (dateStr.Contains("week"))
                            ts = TimeSpan.FromDays(int.Parse(dateStr[0]) * 7);
                        else if (dateStr.Contains("month"))
                            ts = TimeSpan.FromDays(int.Parse(dateStr[0]) * 30);
                        else if (dateStr.Contains("year"))
                            ts = TimeSpan.FromDays(int.Parse(dateStr[0]) * 365);
