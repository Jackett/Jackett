...
namespace Jackett.Common.Models.DTO
{
    ...
    public class Indexer
    {
        ...
        public Indexer(IIndexer indexer)
        {
            ...
            caps = indexer.TorznabCaps.Categories.GroupBy(p => p.ID).Select(g => g.First())
                          .OrderBy(c => c.ID < 100000 ? $"z{c.ID}" : c.Name)
                          .Select(c => new Capability { ID = c.ID.ToString(), Name = c.Name });
        }
    }
}
