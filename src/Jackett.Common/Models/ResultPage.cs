...
namespace Jackett.Common.Models
{
    public class ResultPage
    {
        ...
        private static readonly XNamespace s_TorznabNs = "http://torznab.com/schemas/2015/feed";
        ...
        private XElement getTorznabElement(string name, object value) => value == null
            ? null
            : new XElement(s_TorznabNs + "attr", new XAttribute("name", name), new XAttribute("value", value));
        ...
        public string ToXml(Uri selfAtom)
        {
            var xdoc = new XDocument(
                ...
                new XElement(
                    "rss", new XAttribute("version", "1.0"), new XAttribute(XNamespace.Xmlns + "atom", s_AtomNs.NamespaceName),
                    new XAttribute(XNamespace.Xmlns + "torznab", s_TorznabNs.NamespaceName),
                    new XElement(
                        "channel",
                        new XElement(
                            s_AtomNs + "link", new XAttribute("href", selfAtom.ToString()), new XAttribute("rel", "self"),
                            new XAttribute("type", "application/rss+xml")), new XElement("title", ChannelInfo.Title),
                        new XElement("description", ChannelInfo.Description), new XElement("link", ChannelInfo.Link),
                        new XElement("language", ChannelInfo.Language), new XElement("category", ChannelInfo.Category),
                        new XElement(
                            "image", new XElement("url", ChannelInfo.ImageUrl.ToString()),
                        ...
                        from r in Releases
                        select new XElement(
                            "item", new XElement("title", r.Title), new XElement("guid", r.Guid),
                            ...
                            r.PublishDate == DateTime.MinValue
                                ? new XElement("pubDate", xmlDateFormat(DateTime.Now))
                            ...
                            new XElement("description", r.Description), new XElement("link", r.Link ?? r.MagnetUri),
                            r.Category == null ? null : from c in r.Category select new XElement("category", c),
                            new XElement(
                                "enclosure", new XAttribute("url", r.Link ?? r.MagnetUri),
                            ...
                            getTorznabElement("magneturl", r.MagnetUri), getTorznabElement("rageid", r.RageID),
                            getTorznabElement("thetvdb", r.TVDBId),
                            getTorznabElement("imdb", r.Imdb?.ToString("D7")),
                            getTorznabElement("seeders", r.Seeders), getTorznabElement("peers", r.Peers),
                            getTorznabElement("infohash", r.InfoHash), getTorznabElement("minimumratio", r.MinimumRatio),
