...
namespace Jackett.Common.Models
{
    public class TorznabQuery
    {
        ...
        public bool IsMovieSearch =>
            QueryType == "movie" || (QueryType == "TorrentPotato" && !string.IsNullOrWhiteSpace(SearchTerm));
        ...
        {
            var ret = new TorznabQuery { QueryType = QueryType };
            if (Categories != null && Categories.Length > 0)
            {
        ...
        public string GetEpisodeSearchString()
        {
            ...
            if (DateTime.TryParseExact(
                string.Format("{0} {1}", Season, Episode), "yyyy MM/dd", CultureInfo.InvariantCulture, DateTimeStyles.None,
                out var showDate))
