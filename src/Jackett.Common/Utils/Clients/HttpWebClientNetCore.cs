...
namespace Jackett.Common.Utils.Clients
{
    ...
    public class HttpWebClientNetCore : WebClient
    {
        ...
        public static bool ValidateCertificate(HttpRequestMessage request, X509Certificate2 certificate, X509Chain chain,
        ...
        public HttpWebClientNetCore(IProcessService p, Logger l, IConfigurationService c, ServerConfig sc) : base(
            p, l, c, sc)
        {
            if (webProxyUrl == null)
                InitProxy(sc);
        ...
        public override void Init()
        {
            ...
            {
                logger.Info("HttpWebClient: Disabling certificate validation");
        ...
        protected override async Task<WebClientByteResult> RunAsync(WebRequest webRequest)
        {
            ...
            if (!string.IsNullOrEmpty(webRequest.Cookies))
            {
                ...
                var cookieUrl =
                    new Uri($"{uri.Scheme}://{uri.Host}"); // don't include the path, Scheme is needed for mono compatibility
            ...
            var userAgent = webRequest.EmulateBrowser.Value
                ...
                })
                {
                    ...
                    using (var client = new HttpClient(clearanceHandlr))
                    {
                        ...
                        using (var request = new HttpRequestMessage())
                        {
                            ...
                            {
                                var type = webRequest.Headers.Where(h => h.Key == "Content-Type")
                                                     .Cast<KeyValuePair<string, string>?>().FirstOrDefault();
                                if (type.HasValue)
                                {
                            ...
                            using (response = await client.SendAsync(request))
                            {
                                ...
                                if (result.RedirectingTo?.StartsWith("file://") == true)
                                {
                                    ...
                                    var newRedirectingTo = WebUtilityHelpers.UrlDecode(
                                        result.RedirectingTo, webRequest.Encoding);
                                    newRedirectingTo = newRedirectingTo.StartsWith("file:////")
                                        ? newRedirectingTo.Replace(
                                            "file://", $"{request.RequestUri.Scheme}:")
                                        : newRedirectingTo.Replace(
