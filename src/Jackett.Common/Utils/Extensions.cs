...
namespace Jackett.Common.Utils
{
    ...
    {
        public static IEnumerable<T> ToEnumerable<T>(this T obj) => new[]
        {
            obj
        };
    ...
    public static class CollectionExtension
    {
        ...
        public static bool IsEmptyOrNull<T>(this ICollection<T> obj) => obj?.IsEmpty() != false;
    }
    ...
    public static class ParseExtension
    {
        ...
        {
            var type = typeof(T);
            var parseMethods = type.GetMethods(BindingFlags.Public | BindingFlags.Static).Where(m => m.Name == "Parse");
            var parseMethod = parseMethods.Where(
                m =>
                {
                    var parameters = m.GetParameters();
                    var hasOnlyOneParameter = parameters.Count() == 1;
                    var firstParameterIsString =
    ...
    public static class TaskExtensions
    {
        public static Task<IEnumerable<TResult>> Until<TResult>(this IEnumerable<Task<TResult>> tasks, TimeSpan timeout)
        {
            ...
            var continuation = anyTask.ContinueWith(
                _ =>
                {
