...
namespace Jackett.Common.Utils
{
    public static class TvCategoryParser
    {
        ...
        private static readonly Regex RawHdRegex = new Regex(
            @"\b(?<rawhd>TrollHD|RawHD|1080i[-_. ]HDTV|Raw[-_. ]HD|MPEG[-_. ]?2)\b",
            RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static readonly Regex ResolutionRegex = new Regex(
            @"\b(?:(?<q480p>480p|640x480|848x480)|(?<q576p>576p)|(?<q720p>720p|1280x720)|(?<q1080p>1080p|1920x1080))\b",
            RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private static readonly Regex HighDefPdtvRegex = new Regex(
            @"hr[-_. ]ws", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public static int ParseTvShowQuality(string tvShowFileName)
        {
            ...
            {
                return resolutionMatch.Groups["q1080p"].Success || resolutionMatch.Groups["q720p"].Success
                    ? TorznabCatType.TVHD.ID
                    : TorznabCatType.TVSD.ID;
            }

            if (sourceMatch.Groups["bluray"].Success || sourceMatch.Groups["bdrip"].Success ||
