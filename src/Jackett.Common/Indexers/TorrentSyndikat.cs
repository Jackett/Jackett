...
namespace Jackett.Common.Indexers
{
    public class TorrentSyndikat : BaseWebIndexer
    {
        ...
        private readonly TimeZoneInfo _germanyTz;
        ...
        public TorrentSyndikat(IIndexerConfigurationService configService, WebClient w, Logger l, IProtectionService ps) :
            base(
                "Torrent-Syndikat", description: "A German general tracker", link: "https://torrent-syndikat.org/",
                ...
                configData: new ConfigurationDataBasicLoginWithRSSAndDisplay())
        {
            ...
            _germanyTz = TimeZoneInfo.CreateCustomTimeZone(
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            try
            {
                ...
                {
                    var release = new ReleaseInfo { MinimumRatio = 1, MinimumSeedTime = 96 * 60 * 60 };
                    var qRow = row.Cq();
                    ...
                    var torrentDetails = descCol.Cq().Find(".torrent_details").Get(0);
                    var rawDateStr = torrentDetails.ChildNodes.ElementAt(torrentDetails.ChildNodes.Length - 3).Cq().Text();
                    var dateStr = rawDateStr.Trim().Replace("von", "").Trim();
                    var dateGerman = dateStr.StartsWith("Heute ")
                        ? DateTime.SpecifyKind(DateTime.UtcNow.Date, DateTimeKind.Unspecified) +
                                     TimeSpan.Parse(dateStr.Split(' ')[1])
                        : dateStr.StartsWith("Gestern ")
                        ? DateTime.SpecifyKind(DateTime.UtcNow.Date, DateTimeKind.Unspecified) +
                                     TimeSpan.Parse(dateStr.Split(' ')[1]) - TimeSpan.FromDays(1)
                        : DateTime.SpecifyKind(
                            DateTime.ParseExact(dateStr, "dd.MM.yyyy HH:mm", CultureInfo.InvariantCulture),
                            DateTimeKind.Unspecified);
                    var pubDateUtc = TimeZoneInfo.ConvertTimeToUtc(dateGerman, _germanyTz);
                    ...
                    release.DownloadVolumeFactor = globalFreeleech ? 0 : qRow.Find("span.torrent-tag-free").Length >= 1 ? 0 : 1;
