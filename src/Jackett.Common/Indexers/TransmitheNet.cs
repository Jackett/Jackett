...
namespace Jackett.Common.Indexers
{
    public class TransmitheNet : BaseWebIndexer
    {
        ...
        private string SearchUrl =>
            $"{SiteLink}torrents.php?action=basic&order_by=time&order_way=desc&search_type=0&taglist=&tags_type=0";
        ...
        public TransmitheNet(IIndexerConfigurationService configService, WebClient c, Logger l, IProtectionService ps) :
            base(
                "Nebulance", description: " At Nebulance we will change the way you think about TV",
                link: "https://nebulance.io/", caps: TorznabUtil.CreateDefaultTorznabTVCaps(), configService: configService,
                client: c, logger: l, p: ps,
        ...
        private async Task DoLoginAsync()
        {
            ...
            var response = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, CookieHeader, true, null, LoginUrl);
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            //}
            var response = await RequestStringWithCookiesAndRetryAsync(url);
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    release.PublishDate = !string.IsNullOrEmpty(publishdate) && publishdate.Contains(",")
                        ? DateTime.ParseExact(
                            publishdate, "MMM dd yyyy, HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.AssumeLocal)
                        : DateTime.ParseExact(
                            timeAnchor.TextContent.Trim(), "MMM dd yyyy, HH:mm", CultureInfo.InvariantCulture,
                            DateTimeStyles.AssumeLocal);
                    release.Seeders = ParseUtil.CoerceInt(
                        timeAnchor.ParentElement.NextElementSibling.NextElementSibling.TextContent.Trim());
                    release.Peers = ParseUtil.CoerceInt(
                                        timeAnchor.ParentElement.NextElementSibling.NextElementSibling.NextElementSibling
                                                  .TextContent.Trim()) + release.Seeders;
                    release.Size = ReleaseInfo.GetBytes(timeAnchor.ParentElement.PreviousElementSibling.TextContent);
                    release.MinimumRatio = 1;
                    release.MinimumSeedTime = 172800; // 48 hours
                    release.Files = ParseUtil.CoerceLong(
                        row.QuerySelector("td > div:contains(\"Files:\")").TextContent.Split(':')[1].Trim());
                    release.Grabs = ParseUtil.CoerceLong(row.QuerySelector("td:nth-last-child(3)").TextContent);
                    release.DownloadVolumeFactor = globalFreeleech ? 0 : row.QuerySelector("img[alt=\"Freeleech\"]") != null ? 0 : 1;
