...
namespace Jackett.Common.Indexers
{
    ...
    public class Norbits : BaseCachingWebIndexer
    {
        ...
        public Norbits(IIndexerConfigurationService configService, WebClient w, Logger l, IProtectionService ps) : base(
            "Norbits", description: "Norbits is a Norwegian Private site for MOVIES / TV / GENERAL",
            link: "https://norbits.net/", caps: new TorznabCapabilities(), configService: configService, client: w,
            logger: l, p: ps, configData: new ConfigurationDataNorbits())
        {
            ...
            foreach (var searchTerm in searchTerms)
            {
                ...
                try
                {
                    ...
                    foreach (var tRow in torrentRowList)
                    {
                        ...
                        {
                            subCat2 = tRow.Find("td:eq(0) > div > a[href^=\"/browse.php?sub2_cat[]=\"]").Attr("href")
                                          .Split('?').Last();
                        ...
                        var date = DateTime.ParseExact(
                            dateTime, "yyyy-MM-ddHH:mm:ss", CultureInfo.InvariantCulture,
                        ...
                        {
                            Category = MapTrackerCatToNewznab(testcat),
                            Title = name,
                            Seeders = seeders,
                        ...
                        release.DownloadVolumeFactor = tRow.Find("img[title=\"100% freeleech\"]").Length >= 1
                            ? 0
        ...
        private CQ FindTorrentRows() => _fDom["# torrentTable > tbody > tr:not(:first)"];
