...
namespace Jackett.Common.Indexers
{
    public class BitCityReloaded : BaseWebIndexer
    {
        ...
        public BitCityReloaded(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "Bit-City Reloaded", description: "A German general tracker.", link: "https://bc-reloaded.net/",
                ...
                configData: new ConfigurationDataBasicLoginWithRSSAndDisplay())
        {
            ...
            configData.DisplayText.Value =
                "Only the results from the first search result page are shown, adjust your profile settings to show a reasonable amount (it looks like there's no maximum).";
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    var dateGerman = DateTime.SpecifyKind(
                        DateTime.ParseExact(dateStr, "dd.MM.yyyy HH:mm:ss", CultureInfo.InvariantCulture),
                    ...
                    release.Seeders =
                        ParseUtil.CoerceInt(qRow.Find("table tbody tr:eq(1) td:eq(1) b:eq(0) font").Text().Trim());
                    release.Peers =
                        ParseUtil.CoerceInt(qRow.Find("table tbody tr:eq(1) td:eq(1) b:eq(1) font").Text().Trim()) +
                        release.Seeders;
