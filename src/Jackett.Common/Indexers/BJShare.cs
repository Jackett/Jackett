...
namespace Jackett.Common.Indexers
{
    public class BJShare : BaseWebIndexer
    {
        ...
        public BJShare(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "BJ-Share", description: "A brazilian tracker.", link: "https://bj-share.info/",
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            {
                var results = await RequestStringWithCookiesAsync(TodayUrl);
                if (results.IsRedirect)
                {
                    // re-login
                    await ApplyConfiguration(null);
                    results = await RequestStringWithCookiesAsync(TodayUrl);
                }
                            ...
                            foreach (var child in qBJinfoBox.ChildNodes)
                            {
                                ...
                                else if (line.StartsWith("Lançado em: "))
                                {
                                    ...
                                    var publishDate = DateTime.SpecifyKind(
                                        DateTime.ParseExact(
                                            publishDateStr, "dd/MM/yyyy HH:mm z", CultureInfo.InvariantCulture),
                    ...
                    foreach (var row in rows)
                        try
                        {
                            ...
                            if (row.ClassList.Contains("group_torrent")) // torrents belonging to a group
                            {
                                ...
                                release.Title = categoryStr == "14" ? $"{cleanTitle} {seasonEp}" : $"{cleanTitle} {groupYearStr} {seasonEp}";
                            ...
                            else if (row.ClassList.Contains("torrent")) // standalone/un grouped torrents
                            {
                                ...
                                // do not include year to animes
                                release.Title = categoryStr == "14"
                                    ? $"{cleanTitle} {seasonEp}"
                                    : categoryStr == "2" || categoryStr == "6" ? $"{cleanTitle} {yearStr} {seasonEp}" : $"{cleanTitle} {yearStr}";
                            ...
                            release.Title +=
                                $" {titleElements[5]} {titleElements[3]} {titleElements[1]} {titleElements[2]} {titleElements[4]} {string.Join(" ", titleElements.Skip(6).Take(titleElements.Length - 6).ToArray())}";
