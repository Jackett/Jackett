...
namespace Jackett.Common.Indexers
{
    public class BakaBT : BaseWebIndexer
    {
        private string SearchUrl =>
            $"{SiteLink}browse.php?only=0&hentai=1&incomplete=1&lossless=1&hd=1&multiaudio=1&bonus=1&reorder=1&q=";
        ...
        public BakaBT(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "BakaBT", description: "Anime Comunity", link: "https://bakabt.me/",
            caps: new TorznabCapabilities(TorznabCatType.TVAnime), configService: configService, client: wc, logger: l,
            p: ps,
            configData: new ConfigurationDataBasicLogin(
                "To prevent 0-results-error, Enable the Show-Adult-Content option in your BakaBT account Settings."))
        {
        ...
        {
            var loginForm = await webclient.GetStringAsync(new WebRequest { Url = LoginUrl, Type = RequestType.Get });
            ...
            var response = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, loginForm.Cookies, true, null, SiteLink);
            var responseContent = response.Content;
            await ConfigureIfOkAsync(
                response.Cookies, responseContent.Contains(LogoutStr), () =>
                {
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var release = new ReleaseInfo { Title = (name + releaseInfo).Trim() };
                        ...
                        if (release.Title.IndexOf("Season") == -1)
                        {
                            ...
                            release.Title =
                                $"{release.Title.Substring(0, insertPoint)}Season 1 {release.Title.Substring(insertPoint)}";
                        }
                        ...
                        if (dateStr.Split(' ')[0].Length == 1)
                            dateStr = $"0{dateStr}";
                        release.PublishDate = string.Equals(dateStr, "yesterday", StringComparison.InvariantCultureIgnoreCase)
                            ? DateTime.Now.AddDays(-1)
                            : string.Equals(dateStr, "today", StringComparison.InvariantCultureIgnoreCase)
                            ? DateTime.Now
                            : DateTime.ParseExact(dateStr, "dd MMM yy", CultureInfo.InvariantCulture);
        ...
        {
            var downloadPage = await RequestStringWithCookiesAsync(link.ToString());
            ...
            var response = await RequestBytesWithCookiesAsync(SiteLink + downloadLink);
