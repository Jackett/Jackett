...
namespace Jackett.Common.Indexers
{
    public class Torrentscsv : BaseWebIndexer
    {
        ...
        public Torrentscsv(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "Torrents.csv", description: "Torrents.csv is a self-hostable, open source torrent search engine and database",
            link: "https://torrents-csv.ml/", caps: new TorznabCapabilities(), configService: configService, client: wc,
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            await ConfigureIfOkAsync(
                string.Empty, releases.Count() > 0, () => throw new Exception("Error: No data returned!"));
        ...
        public async Task<IEnumerable<ReleaseInfo>> PerformQueryAsync(TorznabQuery query, int attempts)
        {
            ...
            if (string.IsNullOrEmpty(searchString))
                searchString = "%";
            var queryCollection = new NameValueCollection { { "q", searchString }, { "size", "500" }, { "type_", "torrent" } };
