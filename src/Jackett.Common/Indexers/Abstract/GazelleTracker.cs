...
namespace Jackett.Common.Indexers.Abstract
{
    public abstract class GazelleTracker : BaseWebIndexer
    {
        ...
        protected bool
            supportsCategories =
                true; // set to false if the tracker doesn't include the categories in the API search results
        ...
        public GazelleTracker(IIndexerConfigurationService configService, WebClient webClient, Logger logger,
                              IProtectionService protectionService, string name, string desc, string link,
                              bool supportsFreeleechTokens, bool imdbInTags = false) : base(
            name, description: desc, link: link, caps: new TorznabCapabilities(), configService: configService,
            client: webClient, logger: logger, p: protectionService, configData: new ConfigurationDataBasicLogin())
        {
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            {
                {"username", configData.Username.Value}, {"password", configData.Password.Value}, {"keeplogged", "1"}
            };
            var response = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, string.Empty, true, SiteLink);
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            try
            {
                ...
                foreach (JObject r in json["response"]["results"])
                {
                    ...
                    var release = new ReleaseInfo { PublishDate = groupTime };
                    ...
                    if (imdbInTags)
                    {
                        var imdbTags = tags.Select(tag => ParseUtil.GetImdbID((string)tag)).Where(tag => tag != null);
                        if (imdbTags.Count() == 1)
        ...
        private void FillReleaseInfoFromJson(ReleaseInfo release, JObject torrent)
        {
            ...
            release.Category = category?.Contains("Select Category") != false ? MapTrackerCatToNewznab("1") : MapTrackerCatDescToNewznab(category);
        ...
        public override async Task<byte[]> Download(Uri link)
        {
            ...
            if (content.Length >= 1 && content[0] != 'd' // simple test for torrent vs HTML content
