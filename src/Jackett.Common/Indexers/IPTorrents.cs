...
namespace Jackett.Common.Indexers
{
    public class IPTorrents : BaseWebIndexer
    {
        ...
        public IPTorrents(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "IPTorrents", description: "Always a step ahead.", link: "https://iptorrents.com/",
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            await FollowIfRedirectAsync(response, request.Url, null, firstCallCookies);
            await ConfigureIfOkAsync(
                firstCallCookies, response.Content.Contains("/lout.php"), () =>
                {
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            if (string.IsNullOrWhiteSpace(query.ImdbID) && string.IsNullOrWhiteSpace(query.SearchTerm) &&
                results.Contains("No Torrents Found!"))
                throw new Exception(
            ...
            try
            {
                ...
                foreach (var row in rows.Skip(1))
                {
                    ...
                    if (catIcon.Length >= 1) // Torrents - Category column == Icons
                        ...
                        throw new Exception(
                            $"Please go to {SiteLink}settings.php and change the \"Torrents - Category column\" option to \"Icons\". Wait a minute (cache) and then try again.");
