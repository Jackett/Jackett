...
namespace Jackett.Common.Indexers
{
    public class NCore : BaseWebIndexer
    {
        ...
        private readonly string[] LanguageCats =
        {
            "xvidser",
            "dvdser",
            "hdser",
        ...
        public NCore(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "nCore", description: "A Hungarian private torrent site.", link: "https://ncore.cc/",
                                                ...
                                                int alreadyFounded, int limit, int previouslyParsedOnPage)
        {
            ...
            try
            {
                ...
                for (var i = previouslyParsedOnPage;
                     (i < rows.Length && ((alreadyFounded + releases.Count) < limit));
                     i++)
                    try
                    {
                        var qRow = rows[i].Cq();
                        ...
                        release.Description =
                            $"{qRow.Find("span").Get(0).GetAttribute("title")} {qRow.Find("a.infolink").Text()}";
                        ...
                        release.PublishDate = DateTime.Parse(
                            qRow.Find(".box_feltoltve2").Get(0).InnerHTML.Replace("<br />", " "),
                        ...
                        else
                        {
                            if (query.MatchQueryStringAND(release.Title, null, seasonep))
                            {
                                ...
                                var releasedata = release.Title.Split(
                                    new[]
                                    {
                                        seasonep
                                ...
                                release.Title = (description[0].Trim() + "." + seasonep.Trim() + "." + releasedata.Trim('.'))
                                    .Replace(' ', '.');
        ...
        protected async Task<IEnumerable<ReleaseInfo>> PerformQueryAsync(TorznabQuery query, string seasonep)
        {
            var releases = new List<ReleaseInfo>();
            ...
            var results = await PostDataWithCookiesAndRetryAsync(SearchUrl, pairs);
            CQ dom = results.Content;
            var numVal = 0;

            // find number of torrents / page
            var torrentPerPage = dom[".box_torrent_all"].Find(".box_torrent").Length;
            if (torrentPerPage == 0)
                return releases;
            ...
            if (pagelinks.Length > 0)
                ...
                {
                    var lastPageLink = (pagelinks[i].Cq().Attr("href")).Trim();
            ...
            for (var i = startPage; (i <= numVal && releases.Count < limit); i++)
            {
                ...
                results = await PostDataWithCookiesAndRetryAsync(SearchUrl, pairs);
        ...
        {
            var results = await PerformQueryAsync(query, null);
            if (results.Count() == 0 && query.IsTVSearch
                ) // if we search for a localized title ncore can't handle any extra S/E information, search without it and AND filter the results. See #1450
