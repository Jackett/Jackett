...
namespace Jackett.Common.Indexers
{
    public class ImmortalSeed : BaseWebIndexer
    {
        ...
        public ImmortalSeed(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "ImmortalSeed", description: "ImmortalSeed (iS) is a Private Torrent Tracker for MOVIES / TV / GENERAL",
                link: "https://immortalseed.me/", caps: TorznabUtil.CreateDefaultTorznabTVCaps(),
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            var response = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, null, true, null, LoginUrl);
            CQ resultDom = response.Content;
            await ConfigureIfOkAsync(
                response.Cookies, response.Content.Contains("You have successfully logged in"), () =>
                {
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    release.PublishDate = DateTime.ParseExact(
                        dateString, "MM-dd-yyyy hh:mm tt", CultureInfo.InvariantCulture);
                    ...
                    release.DownloadVolumeFactor = qRow.Find("img[title^=\"Free Torrent\"]").Length >= 1 ? 0 : qRow.Find("img[title^=\"Silver Torrent\"]").Length >= 1 ? 0.5 : 1;
                    release.UploadVolumeFactor = qRow.Find("img[title^=\"x2 Torrent\"]").Length >= 1 ? 2 : 1;
