...
namespace Jackett.Common.Indexers
{
    public class NewRealWorld : BaseWebIndexer
    {
        ...
        public NewRealWorld(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "New Real World", description: "A German general tracker.", link: "https://nrw-tracker.eu/",
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            {
                {"username", configData.Username.Value}, {"password", configData.Password.Value}, {"submit", "Log+in!"}
            };
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            {
                {"showsearch", "1"}, {"incldead", "1"}, {"orderby", "added"}, {"sort", "desc"}
            };
            ...
            try
            {
                ...
                {
                    var release = new ReleaseInfo { MinimumRatio = 0.75, MinimumSeedTime = 0 };
                    var qRow = row.Cq();
                    ...
                    release.DownloadVolumeFactor = qRow.Find("img[title=\"OnlyUpload\"]").Length >= 1 ? 0 : 1;
