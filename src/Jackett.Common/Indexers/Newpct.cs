...
namespace Jackett.Common.Indexers
{
    public class Newpct : BaseCachingWebIndexer
    {
        ...
        private class NewpctRelease : ReleaseInfo
        {
            ...
            public NewpctRelease(NewpctRelease copyFrom) : base(copyFrom)
            {
        ...
        private static readonly Uri DefaultSiteLinkUri = new Uri("https://descargas2020.org");

        private static readonly Uri[] ExtraSiteLinkUris =
        {
            new Uri("https://pctnew.org")
        };

        private static readonly Uri[] LegacySiteLinkUris =
        {
        ...
        private readonly Regex _titleListRegex = new Regex(
            @"Serie( *Descargar)?(.+?)(Temporada(.+?)(\d+)(.+?))?Capitulos?(.+?)(\d+)((.+?)(\d+))?(.+?)-(.+?)Calidad(.*)",
            RegexOptions.IgnoreCase);

        private readonly Regex _titleClassicRegex = new Regex(
            @"(\[[^\]]*\])?\[Cap\.(\d{1,2})(\d{2})([_-](\d{1,2})(\d{2}))?\]", RegexOptions.IgnoreCase);
        ...
        private readonly int[] _allTvCategories = (new[]
        {
            TorznabCatType.TV
        }).Concat(TorznabCatType.TV.SubCategories).Select(c => c.ID).ToArray();

        private readonly int[] _allMoviesCategories = (new[]
        {
            TorznabCatType.Movies
        }).Concat(TorznabCatType.Movies.SubCategories).Select(c => c.ID).ToArray();
        ...
        private readonly string[] _seriesLetterUrls =
        {
            "/series/letter/{0}",
            "/series-hd/letter/{0}"
        };

        private readonly string[] _seriesVoLetterUrls =
        {
            "/series-vo/letter/{0}"
        };
        ...
        private readonly string[] _voUrls =
        {
            "serie-vo",
            "serievo"
        };
        ...
        public override string[] LegacySiteLinks { get; protected set; } =
            LegacySiteLinkUris.Select(u => u.AbsoluteUri).ToArray();

        public Newpct(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "Newpct", description: "Newpct - descargar torrent peliculas, series", link: DefaultSiteLinkUri.AbsoluteUri,
            caps: new TorznabCapabilities(
                TorznabCatType.TV, TorznabCatType.TVSD, TorznabCatType.TVHD, TorznabCatType.Movies),
            configService: configService, client: wc, logger: l, p: ps, configData: new ConfigurationData())
        {
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            lock (cache)
                CleanCache();
            return await PerformQueryAsync(link, query, 0);
        }
        ...
        public override async Task<byte[]> Download(Uri linkParam)
        {
            ...
            foreach (var uri in uris)
            {
                ...
                try
                {
                    var results = await RequestStringWithCookiesAndRetryAsync(uri.AbsoluteUri);
                    await FollowIfRedirectAsync(results);
        ...
        private Uri ExtractDownloadUri(string content, string baseLink)
        {
            foreach (var matcher in _downloadMatchers)
            {
                ...
                if (match.Success)
                {
                    var linkText = matcher.MatchEvaluator != null ? (string)matcher.MatchEvaluator.DynamicInvoke(match) : match.Groups[1].Value;
        ...
        private async Task<IEnumerable<ReleaseInfo>> PerformQueryAsync(Uri siteLink, TorznabQuery query, int attempts)
        {
            ...
            if (rssMode)
            {
                ...
                while (pg <= _maxDailyPages)
                {
                    ...
                    if (validUri != null)
                    {
                        ...
                        results = await RequestStringWithCookiesAndRetryAsync(uri.AbsoluteUri);
                    ...
                    var recentFound = _mostRecentRelease != null && items.Any(
                                          r => r.Title == _mostRecentRelease.Title &&
                                               r.Link.AbsoluteUri == _mostRecentRelease.Link.AbsoluteUri);
                    if (pg == 1)
            ...
            else
            {
                ...
                if (isTvSearch)
                    releases.AddRange(await TvSearchAsync(siteLink, query));
                var isMovieSearch = query.Categories == null || query.Categories.Length == 0 ||
                                    query.Categories.Any(c => _allMoviesCategories.Contains(c));
                if (isMovieSearch)
                    releases.AddRange(await MovieSearchAsync(siteLink, query));
            }

            return releases;
        }

        private async Task<IEnumerable<ReleaseInfo>> TvSearchAsync(Uri siteLink, TorznabQuery query)
        {
            ...
            if (newpctReleases == null)
            {
                ...
                foreach (var seriesListUrl in SeriesListUris(siteLink, seriesName))
                    newpctReleases.AddRange(await GetReleasesFromUriAsync(seriesListUrl, seriesName));
                ...
                r =>
                {
                    ...
                    return (nr.Season.HasValue != season.HasValue || //Can't determine if same season
                            ...
                            (nr.Episode.HasValue != episode.HasValue || //Can't determine if same episode
                             nr.Episode.HasValue && (nr.Episode.Value == episode.Value || //Same episode
                                                     nr.EpisodeTo.HasValue && episode.Value >= nr.Episode.Value &&
                                                     episode.Value <= nr.EpisodeTo.Value //Episode in interval
                             )));
                });
        }

        private async Task<IEnumerable<ReleaseInfo>> GetReleasesFromUriAsync(Uri uri, string seriesName)
        {
            ...
            if (!string.IsNullOrEmpty(seriesEpisodesUrl))
            {
                ...
                while (pg < _maxEpisodesListPages)
                {
                    ...
                    results = await RequestStringWithCookiesAndRetryAsync(episodesListUrl.AbsoluteUri);
                    await FollowIfRedirectAsync(results);
        ...
        {
            var lettersUrl = !_includeVo ? _seriesLetterUrls : _seriesLetterUrls.Concat(_seriesVoLetterUrls);
        ...
        private IEnumerable<NewpctRelease> ParseDailyContent(string content)
        {
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    _dailyResultIdx++;
                    var newpctRelease = releaseType == ReleaseType.Tv
                        ? GetReleaseFromData(
                            releaseType, string.Format("Serie {0} - {1} Calidad [{2}]", title, language, quality),
                            detailsUrl, quality, language, ReleaseInfo.GetBytes(sizeText),
                            _dailyNow - TimeSpan.FromMilliseconds(_dailyResultIdx))
                        : GetReleaseFromData(
                            releaseType, string.Format("{0} [{1}][{2}]", title, quality, language), detailsUrl, quality,
                            language, ReleaseInfo.GetBytes(sizeText),
                            _dailyNow - TimeSpan.FromMilliseconds(_dailyResultIdx));
        ...
        private string ParseSeriesListContent(string content, string title)
        {
            ...
            try
            {
                ...
                foreach (var anchor in rows)
                {
                    ...
                    if (string.Equals(h2.TextContent.Trim(), title.Trim(), StringComparison.CurrentCultureIgnoreCase))
        ...
        private async Task<IEnumerable<ReleaseInfo>> MovieSearchAsync(Uri siteLink, TorznabQuery query)
        {
            var releases = new List<NewpctRelease>();
            ...
            while (pg <= _maxMoviesPages)
            {
                ...
                {
                    {"q", searchStr}, {"s", searchStr}, {"pg", pg.ToString()}
                };
                WebClientStringResult results = null;
                IEnumerable<NewpctRelease> items = null;
                if (validUri != null)
                {
                    if (validUriUsesJson)
                    {
                        ...
                        results = await PostDataWithCookiesAsync(uri.AbsoluteUri, queryCollection);
                        if (string.IsNullOrEmpty(results?.Content))
                            break;
                    ...
                    using (var uris = GetLinkUris(new Uri(siteLink, _searchUrl)).GetEnumerator())
                    {
                        ...
                        while (jsonUris.MoveNext() && uris.MoveNext() && !resultFound)
                            ...
                            {
                                var usingJson = i == 0;
                                var uri = usingJson ? jsonUris.Current : uris.Current;
                                try
                                {
                                    results = await PostDataWithCookiesAsync(uri.AbsoluteUri, queryCollection);
                                }
                                ...
                                if (!string.IsNullOrEmpty(results?.Content))
                                {
                                    items = usingJson ? ParseSearchJsonContent(uri, results.Content) : ParseSearchContent(results.Content);
                                    if (items != null)
                                    {
                                        validUri = uri;
        ...
        private IEnumerable<NewpctRelease> ParseSearchContent(string content)
        {
            ...
            return !someFound ? null : releases;
        }
        ...
        private IEnumerable<NewpctRelease> ParseSearchJsonContent(Uri uri, string content)
        {
            ...
            var ub = new UriBuilder(uri) { Path = string.Empty };
            uri = ub.Uri;
            try
            {
                ...
                for (var i = 0; i < numItems; i++)
                {
                    ...
                    someFound = true;
                    var isSeries = calidad?.ToLower().Contains("hdtv") == true;
                    var isGame = title.ToLower().Contains("pcdvd");
                    if (isSeries || isGame)
                        continue;
                    long size = 0;
                    ...
                    newpctRelease = GetReleaseFromData(
                        ReleaseType.Movie, title, detailsUrl, calidad, null, size, publishDate);
                    releases.Add(newpctRelease);
                }
            }
            catch (Exception)
        ...
        {
            var searchWords = searchTerm.ToLower().Split(_wordSeparators, StringSplitOptions.None).Select(s => s.Trim())
        ...
        private ReleaseType ReleaseTypeFromQuality(string quality) => quality.Trim().ToLower().StartsWith("hdtv") ? ReleaseType.Tv : ReleaseType.Movie;
        ...
        {
            var result = new NewpctRelease { NewpctReleaseType = releaseType };
            ...
            if (match.Success)
            {
                ...
                var episodeToText = result.EpisodeTo.HasValue
                    ? $"_{seasonText}{result.EpisodeTo.ToString().PadLeft(2, '0')}"
                    : "";
                result.Title = string.Format(
                    "{0} - Temporada {1} [{2}][Cap.{3}{4}][{5}]", result.SeriesName, seasonText, quality, episodeText,
                    episodeToText, audioQuality);
            }
            else
            {
            ...
            {
                result.Category = !string.IsNullOrWhiteSpace(quality) && (quality.Contains("720") || quality.Contains("1080"))
        ...
        private string FixedTitle(NewpctRelease release, string quality, string language)
        {
            ...
            var titleParts = new List<string> { release.SeriesName };
            ...
            if (release.Title.ToLower().Contains("espa\u00F1ol") || release.Title.ToLower().Contains("espanol") ||
                release.Title.ToLower().Contains("castellano") || release.Title.ToLower().EndsWith("espa"))
