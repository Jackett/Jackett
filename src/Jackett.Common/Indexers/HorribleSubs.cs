...
namespace Jackett.Common.Indexers
{
    internal class HorribleSubs : BaseWebIndexer
    {
        ...
        public HorribleSubs(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "Horrible Subs", description: "HorribleSubs - So bad yet so good", link: "https://horriblesubs.info/",
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            if (string.IsNullOrWhiteSpace(searchString))
                return await PerformLatestQueryAsync(query);
            ...
            try
            {
                ...
                {
                    var href = SiteLink + resultLink.Attributes["href"].Value
                                                    .Substring(
                                                        1); // = https://horriblesubs.info/shows/boruto-naruto-next-generations#71
                ...
                {
                    var showReleases = await GetReleasesAsync(showLink, false);
        ...
        private async Task<IEnumerable<ReleaseInfo>> PerformLatestQueryAsync(TorznabQuery query)
        {
            var resultParser = new HtmlParser();
            var releases = new List<ReleaseInfo>();
            var searchString = query.GetQueryString();
            ...
            try
            {
                ...
                foreach (var resultLink in latestresults)
                {
                    ...
                    var showUrl = href.Substring(0, href.LastIndexOf("#"));
                    var showReleases = await GetReleasesAsync(showUrl, true, episodeNumber);
                    releases.AddRange(showReleases);
                }
            }
            catch (Exception ex)
        ...
        private async Task<IEnumerable<ReleaseInfo>> GetReleasesAsync(string resultUrl, bool latestOnly,
                                                                 string titleContains = null)
        {
            ...
            try
            {
                var showPageResponse = await RequestStringWithCookiesAndRetryAsync(resultUrl, string.Empty);
                await FollowIfRedirectAsync(showPageResponse);
                ...
                var releaserows = new List<IElement>();
                foreach (var apiUrl in apiUrls)
                {
                    var nextId = 0;
                    while (true)
                    {
                        var showApiResponse = await RequestStringWithCookiesAndRetryAsync($"{apiUrl}&nextid={nextId}", string.Empty);
