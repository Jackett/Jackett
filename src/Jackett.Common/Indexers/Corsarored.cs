...
namespace Jackett.Common.Indexers
{
    public class Corsarored : BaseWebIndexer
    {
        ...
        public Corsarored(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "Corsaro.red", description: "Italian Torrents", link: "https://corsaro.red/", caps: new TorznabCapabilities(),
        ...
        private async Task<dynamic> SendApiRequestAsync(IEnumerable<KeyValuePair<string, string>> data)
        {
            var result = await PostDataWithCookiesAndRetryAsync(ApiSearch, data, null, SiteLink, _apiHeaders, null, true);
            return CheckResponse(result);
        }

        private async Task<dynamic> SendApiRequestLatestAsync()
        {
            var result = await RequestStringWithCookiesAndRetryAsync(ApiLatest, null, SiteLink, _apiHeaders);
            return CheckResponse(result);
        }
        ...
        private string GetApiCategory(TorznabQuery query)
        {
            ...
            if (cats.Count == 0)
                return "0";
            string apiCat = null;
            foreach (var cat in cats.Select(cat => _apiCategories.FirstOrDefault(kvp => kvp.Value.Contains(cat)).Key))
            {
                if (apiCat == null)
                    apiCat = cat;
                if (apiCat != cat)
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            if (string.IsNullOrWhiteSpace(searchString))
            {
                ...
                var result = await SendApiRequestLatestAsync();
                try
                {
            ...
            for (var page = 1; page <= MaxSearchPageLimit; page++)
            {
                ...
                var result = await SendApiRequestAsync(queryCollection);
                try
                {
