...
namespace Jackett.Common.Indexers
{
    public class TorrentBytes : BaseWebIndexer
    {
        ...
        public TorrentBytes(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "TorrentBytes", description: "A decade of torrentbytes", link: "https://www.torrentbytes.net/",
                caps: TorznabUtil.CreateDefaultTorznabTVCaps(), configService: configService, client: wc, logger: l, p: ps,
                configData: new ConfigurationDataBasicLogin(
                    "For best results, change the 'Torrents per page' setting to 30 or greater (100 recommended) in your profile on the TorrentBytes webpage."))
        {
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            var loginPage = await RequestStringWithCookiesAsync(SiteLink, string.Empty);
            var result = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, loginPage.Cookies, true, SiteLink, SiteLink);
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            await ProcessPageAsync(releases, searchUrl);
            return releases;
        }

        private async Task ProcessPageAsync(List<ReleaseInfo> releases, string searchUrl)
        {
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    release.DownloadVolumeFactor = row.Cq().Find("font[color=\"green\"]:contains(F):contains(L)").Length >= 1 ? 0 : 1;
