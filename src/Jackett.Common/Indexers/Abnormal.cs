...
namespace Jackett.Common.Indexers
{
    ...
    public class Abnormal : BaseCachingWebIndexer
    {
        ...
        private string TorrentDownloadUrl =>
            $"{SiteLink}torrents.php?action=download&id={{id}}&authkey={{auth_key}}&torrent_pass={{torrent_pass}}";
        ...
        private static string Directory => Path.Combine(
            Path.GetTempPath(), Assembly.GetExecutingAssembly().GetName().Name.ToLower(),
            MethodBase.GetCurrentMethod().DeclaringType?.Name.ToLower());
        ...
        public Abnormal(IIndexerConfigurationService configService, WebClient w, Logger l, IProtectionService ps) : base(
            "Abnormal", description: "General French Private Tracker", link: "https://abnormal.ws/",
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            var myRequest = new WebRequest { Url = LoginUrl };

            // Add our headers to request
            myRequest.Headers = _emulatedBrowserHeaders;
            ...
            await ConfigureIfOkAsync(
                response.Cookies, response.Cookies.Contains("session="), () =>
                {
                    // Parse error page
                    ...
                    output(
                        $"-> Login failed: \"{message}\" and {left} tries left before being banned for 6 hours !",
                        "error");
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            var franceTz = TimeZoneInfo.CreateCustomTimeZone(
            ...
            _fDom = await QueryExecAsync(request);
            try
            {
                // Find torrent rows
                ...
                var pagination = (_fDom[".linkbox > a"].Length != 0);
                ...
                if (pagination)
                {
                    ...
                    pageLinkCount = ParseUtil.CoerceInt(Regex.Match(_fDom[".linkbox > a"].Last().Attr("href"), @"\d+").Value);
                ...
                if (!string.IsNullOrWhiteSpace(query.GetQueryString()) && pageLinkCount > 1)
                    ...
                    for (var i = 2; i <= Math.Min(int.Parse(ConfigData.Pages.Value), pageLinkCount); i++)
                    {
                        ...
                        // Getting results & Store content
                        _fDom = await QueryExecAsync(pageRequest);
                ...
                foreach (var tRow in torrentRowList)
                {
                    ...
                    // ID
                    var id = ParseUtil.CoerceInt(Regex.Match(tRow.Find("td:eq(1) > a").Attr("href"), @"\d+").Value);
                    output($"ID: {id}");
                    ...
                    output($"Category: {MapTrackerCatToNewznab(categoryId).First()} ({categoryId})");
                    ...
                    var dateLocal = DateTime.SpecifyKind(
                        DateTime.ParseExact(datestr, "MMM dd yyyy, HH:mm", CultureInfo.InvariantCulture),
        ...
        /// <param name="request">URL created by Query Builder</param>
        /// <returns>Results from query</returns>
        private async Task<string> QueryCacheAsync(string request)
        {
            string results;
        ...
        private async Task<string> QueryTrackerAsync(string request)
        {
            ...
            latencyNow();
            var results = await RequestStringWithCookiesAndRetryAsync(request, null, null, _emulatedBrowserHeaders);
        ...
        private CQ findTorrentRows() => _fDom[".torrent_table > tbody > tr"].Not(".colhead");
