...
namespace Jackett.Common.Indexers
{
    public class RevolutionTT : BaseWebIndexer
    {
        ...
        public RevolutionTT(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) :
            base(
                "RevolutionTT", description: "The Revolution has begun", link: "https://revolutiontt.me/",
                caps: TorznabUtil.CreateDefaultTorznabTVCaps(), configService: configService, client: wc, logger: l, p: ps,
                downloadBase: "https://revolutiontt.me/download.php/", configData: new ConfigurationDataBasicLoginWithRSS())
        {
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            var homePageLoad = await RequestLoginAndFollowRedirectAsync(
                LandingPageURL, new Dictionary<string, string>(), null, true, null, SiteLink);
            var result = await RequestLoginAndFollowRedirectAsync(
                LoginUrl, pairs, homePageLoad.Cookies, true, null, LandingPageURL);
            ...
            try
            {
                var rssParams = new Dictionary<string, string> { { "feed", "dl" } };
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            {
                var rssPage = await RequestStringWithCookiesAndRetryAsync(RSSUrl + configData.RSSKey.Value);
                ...
                foreach (var item in rssDoc.Descendants("item"))
                {
                    ...
                    var publishDate = DateTime.ParseExact(
                        date, "ddd, dd MMM yyyy HH:mm:ss zz00", CultureInfo.InvariantCulture);
                    ...
                    var release = new ReleaseInfo
                    {
                        ...
                        Seeders =
                            ParseUtil.CoerceInt(
                                infoMatch.Groups["seeders"].Value == "no" ? "0" : infoMatch.Groups["seeders"].Value),
                        Peers =
                            ParseUtil.CoerceInt(
                                infoMatch.Groups["leechers"].Value == "no" ? "0" : infoMatch.Groups["leechers"].Value),
                    ...
                    foreach (var row in qRows)
                    {
                        ...
                        if (releaseLinkUri != null)
                        {
                            ...
                            release.PublishDate = DateTime.ParseExact(
                                dateString, "yyyy-MM-ddHH:mm:ss", CultureInfo.InvariantCulture);
