...
namespace Jackett.Common.Indexers
{
    public class HDSpace : BaseWebIndexer
    {
        ...
        public HDSpace(IIndexerConfigurationService configService, WebClient wc, Logger l, IProtectionService ps) : base(
            "HD-Space", description: "Sharing The Universe", link: "https://hd-space.org/",
            ...
            configData: new ConfigurationDataBasicLogin())
        {
            ...
            AddMultiCategoryMapping(
                TorznabCatType.MoviesHD, 19, // Movie / 1080p
                41, // Movie / 4K UHD
                18, // Movie / 720p
                40, // Movie / Remux
                16 // Movie / HD-DVD
            ...
            AddMultiCategoryMapping(
                TorznabCatType.TVHD, 21, // TV Show / 720p HDTV
            ...
            AddMultiCategoryMapping(
                TorznabCatType.TVDocumentary, 24, // TV Show / Documentary / 720p
                25 // TV Show / Documentary / 1080p
                );
            AddMultiCategoryMapping(
                TorznabCatType.XXX, 33, // XXX / 720p
                34 // XXX / 1080p
                );
        ...
        public override async Task<IndexerConfigurationStatus> ApplyConfiguration(JToken configJson)
        {
            ...
            {
                {"uid", configData.Username.Value}, {"pwd", configData.Password.Value}
            };

            // Send Post
            var response = await RequestLoginAndFollowRedirectAsync(LoginUrl, pairs, loginPage.Cookies, true, null, LoginUrl);
        ...
        protected override async Task<IEnumerable<ReleaseInfo>> PerformQuery(TorznabQuery query)
        {
            ...
            {
                {"active", "0"}, {"options", "0"}, {"category", string.Join(";", MapTorznabCapsToTrackers(query))}
            };
            ...
            try
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    release.PublishDate = dateStr.StartsWith("Today")
                        ? DateTime.Today + TimeSpan.ParseExact(
                                                  dateStr.Replace("Today at ", ""), "hh\\:mm\\:ss",
                                                  CultureInfo.InvariantCulture)
                        : dateStr.StartsWith("Yesterday")
                        ? DateTime.Today - TimeSpan.FromDays(1) + TimeSpan.ParseExact(
                                                  dateStr.Replace("Yesterday at ", ""), "hh\\:mm\\:ss",
                                                  CultureInfo.InvariantCulture)
                        : DateTime.SpecifyKind(
                            DateTime.ParseExact(dateStr, "MMMM dd, yyyy, HH:mm:ss", CultureInfo.InvariantCulture),
                            DateTimeKind.Local);
                    ...
                    release.DownloadVolumeFactor = qRow.Find("img[title=\"FreeLeech\"]").Length >= 1
                        ? 0
                        : qRow.Find("img[src=\"images/sf.png\"]").Length >= 1 ? 0 : qRow.Find("img[title=\"Half FreeLeech\"]").Length >= 1 ? 0.5 : 1;
