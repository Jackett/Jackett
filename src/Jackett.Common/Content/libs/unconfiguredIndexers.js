...
export function displayUnconfiguredIndexersList() {
    ...
    indexersTable.find('.indexer-setup').each((i, btn) => {
        const indexer = utils.indexers.unconfigured[i];
        btn.addEventListener('click', () => {
            modal.hide();
            displayIndexerSetup(indexer);
        });
    });

    indexersTable.find('.indexer-add').each((i, btn) => {
        const indexer = utils.indexers.unconfigured[i];
        $(btn).on('click', () => {
            modal.hide();
            modalElement.addEventListener('hidden.bs.modal', (_) => {
                addIndexer(indexer.id, true);
            }, { once: true });
        });
    });
...
function addIndexer(indexerId, displayNotification) {
    api.getIndexerConfig(indexerId, data => {
        ...
        api.updateIndexerConfig(indexerId, data, data => {
            ...
            utils.notifyError(indexerId, data.responseJSON.error, "configuring");
        });
    });
}
