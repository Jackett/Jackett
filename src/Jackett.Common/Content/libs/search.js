...
export function displaySearch(selectedFilter, selectedIndexer, query, category) {
    ...
    releaseDialog.addEventListener('hidden.bs.modal', () => {
        const input = document.querySelector('#indexers div.dataTables_filter input');
    ...
    searchButton.addEventListener('click', () => {
        ...
        const searchResultsInput = document.querySelector('#searchResults div.dataTables_filter input');
        ...
        api.resultsForIndexer(trackerId, queryObj, (data) => {
            ...
            const input = searchResults.querySelector('div.dataTables_filter input');
            if (input) utils.stableFocus(input);
...
$.fn.DataTable.ext.search.push((settings, data, dataIndex) => {
    ...
    if (!deadfiltercheckbox.checked && seeders == 0)
        return false;
    return true;
});
    ...
    return $(tableElement).DataTable({
        ...
        initComplete() {
            ...
            this.api().columns().every(function() {
                ...
                const uniqueData = column.data().toArray().filter((item, pos, self) => self.indexOf(item) === pos).sort();
