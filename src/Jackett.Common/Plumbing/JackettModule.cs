...
namespace Jackett.Common.Plumbing
{
    public class JackettModule : Module
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            builder.RegisterAssemblyTypes(typeof(JackettModule).Assembly).Except<IIndexer>().Except<IImdbResolver>()
                   .Except<OmdbResolver>().Except<IFallbackStrategyProvider>().Except<ImdbFallbackStrategyProvider>()
                   .Except<IFallbackStrategy>().Except<ImdbFallbackStrategy>().Except<IResultFilterProvider>()
                   .Except<ImdbTitleResultFilterProvider>().Except<IResultFilter>().Except<ImdbTitleResultFilterProvider>()
                   .Except<BaseMetaIndexer>().Except<AggregateIndexer>().Except<CardigannIndexer>()
                   .AsImplementedInterfaces().SingleInstance();
            builder.RegisterInstance(_runtimeSettings);
            builder.Register(
                ctx => BuildServerConfig(ctx)).As<ServerConfig>().SingleInstance();
        ...
        private void RegisterWebClient<TWebClientType>(ContainerBuilder builder) =>
            builder.RegisterType<TWebClientType>().As<WebClient>();
                ...
                if (monotype != null)
                {
                    ...
                    if (displayName != null)
                    {
                        ...
                        if ((monoVersionO.Major >= 4 && monoVersionO.Minor >= 8) || monoVersionO.Major >= 5)
                        {
                            ...
                            var monoTlsProviderFactory =
                                monoSecurity.GetType("Mono.Security.Interface.MonoTlsProviderFactory");
                            if (monoTlsProviderFactory != null)
                            {
                                ...
                                {
                                    var btlsSupported = (bool)isProviderSupported.Invoke(
                                        null, new[]
                                        {
                                            "btls"
                                        });
                                    ...
                                    {
                                        // initialize btls
                                        var initialize = monoTlsProviderFactory.GetMethod(
                                            "Initialize", new[]
                                            {
                                                typeof(string)
                                            });
