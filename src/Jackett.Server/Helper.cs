...
namespace Jackett.Server
{
    public static class Helper
    {
        ...
        private static bool s_AutomapperInitialised;
        ...
        {
            if (s_AutomapperInitialised == false)
            {
        ...
        private static void InitAutomapper() => Mapper.Initialize(
            cfg =>
            {
                cfg.CreateMap<WebClientByteResult, WebClientStringResult>().ForMember(x => x.Content, opt => opt.Ignore())
                   .AfterMap(
                       (be, str) =>
                       {
                ...
                cfg.CreateMap<WebClientStringResult, WebClientByteResult>().ForMember(x => x.Content, opt => opt.Ignore())
                   .AfterMap(
                       (str, be) =>
                       {
                ...
                cfg.CreateMap<ReleaseInfo, TrackerCacheResult>().AfterMap(
                    (r, t) => t.CategoryDesc = r.Category != null
                            ? string.Join(
                                ", ",
                                r.Category.Select(x => TorznabCatType.GetCatDesc(x))
